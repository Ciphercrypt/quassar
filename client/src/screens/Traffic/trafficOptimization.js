...
const DadarMapOpt = () => {
  ...
  const getData = async () => {
    const response = await axios.get(
      "http://localhost:5000/api/scheduling/getSchedulingScore"
    );

    setPolyData(response.data);

    console.log("PolyData",response.data);
    setRoadData(response.data[0]);
    setSignalKeys(Object.keys(response.data.data[0]));
    console.log("signalkeys",signalkeys);
